...
local function generate_map_legend_data()
    global.Map_legend_data = {entities = {}, resources = {}, trees = {}, tiles = {}, iconMap = {}}
    ...
    if settings.startup["custom-map-colors-preset"].value == "None" then
        Map_legend_data.entities["pipe-to-ground"] = {r = 25, g = 103, b = 150}
        Map_legend_data.entities["pump"] = {r = 109, g = 154, b = 181}
        Map_legend_data.entities["storage-tank"] = {r = 131, g = 166, b = 188}
    ...
    for _,v in pairs(supportedEntities.utilConstants) do
        ...
        Map_legend_data.entities[name] = colorLib.toColor(settings.startup["use-custom-"..v.."-color"].value and settings.startup["custom-"..v.."-color"].value or activePreset[v] or defaultPreset[v])
    end

    for name,resource in pairs(game.get_filtered_entity_prototypes({{filter = "type", type = "resource"}})) do
        Map_legend_data.resources[name] = colorLib.toColor(resource.map_color)
    end
    for name,tree in pairs(game.get_filtered_entity_prototypes({{filter = "type", type = "tree"}})) do
        Map_legend_data.trees[name] = colorLib.toColor(tree.map_color)
    end


    local iconMap = Map_legend_data.iconMap
    for k,_ in pairs(Map_legend_data.entities) do
        iconMap[k] = "[img=entity/"..k.."]"
    end
...
local function create_tab(parent, name, data, locale_type)
    ...
    local tab_outline = parent.add({type = 'frame', name = name..'tab_outline', style = 'window_content_frame_packed', direction = 'vertical'})
...
local function create_map_legend(player)
    ...
    local tabbed_pane_outline = legend.add({type = 'frame', name = 'tabbed_pane_outline', style = 'inside_deep_frame_for_tabs', direction = 'vertical'})
        local tabbed_pane = tabbed_pane_outline.add{type="tabbed-pane"}
...
local on_gui_location_changed = function( event )
    ...
    if element.name == "map_legend" then
        global.Position[event.player_index] = element.location
    end
end
